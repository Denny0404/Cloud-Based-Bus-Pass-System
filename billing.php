...
// Logic Functions
function insertPass($con, $name, $email, $contact, $date, $dest, $password) {
    $stmt = $con->prepare("INSERT INTO pass(name, email, contact, date, dest, password) VALUES (?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssss", $name, $email, $contact, $date, $dest, $password);
    $stmt->execute();
    return $con->insert_id;
}

function calculateDaysFromToday($date) {
    return round((strtotime($date) - time()) / (60 * 60 * 24)) + 1;
}

function getPriceForDestination($con, $dest) {
...
function calculateAmount($price, $days) {
    return $price * $days;
}
...
if (!defined('PHPUNIT_RUNNING')) {
    if (isset($_POST['name'])) {
        ...
        $lid = insertPass($con, $name, $email, $contact, $date, $dest, $password);
        $nod = calculateDaysFromToday($date);
        $price = getPriceForDestination($con, $dest);
        $amt = calculateAmount($price, $nod);
    }
?>
...
<div class="site-wrap">
  ...
  <style>
    ...
    .col-50 { flex: 50%; }
    .col-75 { flex: 75%; }
    .container { padding: 5px 20px 15px 20px; border-radius: 3px; }
    ...
    .btn {
      ...
      margin: 10px 0; border: none; width: 100%;
          ...
          <div class="row">
            <div class="col-50">
              ...
              <div class="icon-container">
                ...
                <div>Rs. <?php if (isset($amt)) echo $amt; ?></div>
              </div>
              <input type="hidden" value="<?php if (isset($lid)) echo $lid; ?>" name="id">
              <input type="hidden" value="<?php if (isset($amt)) echo $amt; ?>" name="amt">
              <label for="cname">Name on Card</label>
              <input type="text" id="cname" name="cardname" value="<?php if (isset($name)) echo $name; ?>" required>
