...
function updatePayment($con, $id, $amt) {
    $stmt = $con->prepare("UPDATE pass SET paid = paid + ? WHERE id = ?");
    $stmt->bind_param("di", $amt, $id);
    return $stmt->execute();
}

function getPassDetails($con, $id) {
...
// Only run UI logic if not under PHPUnit
if (!defined('PHPUNIT_RUNNING')) {
    if (isset($_POST['id'])) {
        $id = $_POST['id'];
        $amt = $_POST['amt'];

        updatePayment($con, $id, $amt);
        $row = getPassDetails($con, $id);
    }
?>
<!DOCTYPE html>
<html lang="en">
