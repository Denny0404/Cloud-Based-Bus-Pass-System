...
function updatePassDate($con, $id, $newDate) {
    $stmt = $con->prepare("UPDATE pass SET date = ? WHERE id = ?");
    $stmt->bind_param("si", $newDate, $id);
    return $stmt->execute();
}

function calculateNewDays($oldDate, $newDate) {
    return round((strtotime($newDate) - strtotime($oldDate)) / (60 * 60 * 24)) + 1;
}

function fetchPriceForDestination($con, $dest) {
...
if (!defined('PHPUNIT_RUNNING')) {
    if (isset($_POST['id'])) {
        ...
        $row = fetchPassById($con, $id);
        updatePassDate($con, $id, $new_date);
        $nod = calculateNewDays($row['date'], $new_date);

        $dest = $row['dest'];
        $price = fetchPriceForDestination($con, $dest);
        $amt = $price * $nod;
    }
?>
